...
class BookEntity {
	...
	static BookEntity from(Book book) {
		...
		return new BookEntity(book.id().getValue(), book.title().asString(), book.author().asString(),
				book.isbn().asString(), patronId, BookEntityStatus.of(book), book.version().asLong());
	}
